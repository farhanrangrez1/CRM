...
//                 "Lost": "lost"
//             };
//             return map[status] || "serviceCalls";
//         }
...
//             needPayment: reduxProposals.filter(p => p.status === "Need Payment"),
//             needToPayDWP: reduxProposals.filter(p => p.status === "Need to Pay DWP"),
//             needToInstall: reduxProposals.filter(p => p.status === "Need to Install"),
//             inspectionScheduled: reduxProposals.filter(p => p.status === "Inspection Scheduled"),
//             passedInspection: reduxProposals.filter(p => p.status === "Passed Inspection"),
//             waitingOnMeter: reduxProposals.filter(p => p.status === "Waiting on Meter"),
//             removeBreakers: reduxProposals.filter(p => p.status === "Remove Breakers"),
//             meterInstalled: reduxProposals.filter(p => p.status === "Meter Installed"),
//             requestedToRemove: reduxProposals.filter(p => p.status === "Requested to Remove"),
//             tempPoleRemoved: reduxProposals.filter(p => p.status === "Temp Pole Removed"),
//             tempPoleOnly: reduxProposals.filter(p => p.status === "Temp Pole Only"),
...
//                             <h5 className="fw-bold mb-0" style={{ fontSize: '2rem' }}>Temp Poles</h5>
//                         </div>
//                         {/* Search and Actions */}
//                         <div className="mb-4">
...
const tabs = [
    ...
    { label: 'Need Payment', value: 'needPayment' },
    { label: 'Need to Pay DWP', value: 'needToPayDWP' },
    { label: 'Need to Install', value: 'needToInstall' },
    { label: 'Inspection Scheduled', value: 'inspectionScheduled' },
    { label: 'Passed Inspection', value: 'passedInspection' },
    { label: 'Waiting on Meter', value: 'waitingOnMeter' },
    { label: 'Remove Breakers', value: 'removeBreakers' },
    { label: 'Meter Installed', value: 'meterInstalled' },
    { label: 'Requested to Remove', value: 'requestedToRemove' },
    { label: 'Temp Pole Removed', value: 'tempPoleRemoved' },
    { label: 'Temp Pole Only', value: 'tempPoleOnly' },
...
const TempPoles = ({ data }) => {
    ...
    const ProposalWorkflowBoard = ({ onNavigate, selectedStatus }) => {
        ...
        function mapStatus(status) {
            ...
            return map[status] || "open"; // fallback
        }
                                ...
                                <div className="nav nav-tabs d-none d-md-flex">
                                    {tabs.map((tab) => (
                                        <li className="nav-item" key={tab.value}>
                                            <button
                                                className={`nav-link ${selectedStatus === tab.value ? 'active' : ''}`}
                                                onClick={() => handleTabClick(tab.value)} // Use the new handler
                                                style={{ color: "#695e13", borderColor: "#DDC62F" }}
                                            >
                                                {tab.label}
                                            </button>

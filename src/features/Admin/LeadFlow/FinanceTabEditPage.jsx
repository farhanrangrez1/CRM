...
// export default FinanceTabEditPage;
        ...
        <div
            ...
            <div className="table-responsive">
                <Table className="align-middle sticky-header">
                    ...
                    <tbody>
                        {envelopdata?.map((item, index) => (
                            <tr key={item._id || index}>
                                <td>{project?.data.find(p => p._id === item.project_id)?.projectName || "N/A"}</td>
                                <td>{project?.data.find(p => p._id === item.project_id)?.clientId?.clientName || "N/A"}</td>
                                <td>{item.email}</td>
                                <td>
                                    {item.attachment?.map((link, i) => (
                                        <a key={i} href={link} target="_blank" rel="noreferrer">Download {i + 1}</a>
                                    ))}
                                </td>
                            </tr>
                        ))}
